...
function usePlayer(id?: string, url?: string, metaData?: EventMetaData) {
    ...
    useEffect(() => {
        if (id && !audioPlayer.playing(null)) {
            ...
            (async (): Promise<void> => {
                await audioPlayer.init({ id, url: url || '', metaData });
            })();
        }
    ...
    }, [id, url, ...(Object.values(metaData || {}) as unknown[])]);
    ...
    const togglePlayback = useCallback(() => {
        ...
        if (isPlaying) {
            ...
            if (id) {
                void audioPlayer.play({ id, url: url || '', metaData });
            } else {
    ...
    }, [audioPlayer.playingStartTime, id, url, ...(Object.values(metaData || {}) as unknown[])]);
...
export function Playbar(props: PlaybarProps): ReactElement | null {
    const { hideEventDetails, hidePlayer, id, url, metaData, showFullDetails } = props;
    const {
        audioPlayer,
        isActive,
        isPlaying,
