...
describe('SettingsButton', () => {
    ...
    test('renders Open Aiera Dash link when showDashButton config is true', async () => {
        const config: Config = {
            assetPath: 'assets',
    ...
    test('does not render Open Aiera Dash link when showDashButton config is false', async () => {
        const config: Config = {
            assetPath: 'assets',
            platform: 'aiera-sdk-dev',
            gqlOptions: { clientOptions: { url: 'test' } },
            showDashButton: false,
        };
        const TestComponent = () => {
            return (
                <Provider config={config}>
