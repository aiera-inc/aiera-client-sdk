...
describe('Toggle', () => {
    test('renders', () => {
        ...
        const { rendered } = renderWithProvider(<Toggle onChange={toggleFn} />);
    ...
    test('renders true', () => {
        const toggleFn = jest.fn();
        const { rendered } = renderWithProvider(<Toggle onChange={toggleFn} on={true} />);
        const toggleElement = rendered.container.querySelector('.toggle') as HTMLInputElement;
        expect(toggleElement.checked).toEqual(true);
    });
    test('renders false', () => {
        const toggleFn = jest.fn();
        const { rendered } = renderWithProvider(<Toggle onChange={toggleFn} on={false} />);
        const toggleElement = rendered.container.querySelector('.toggle') as HTMLInputElement;
