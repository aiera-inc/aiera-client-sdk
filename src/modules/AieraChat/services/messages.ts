...
import { TextBlock } from '@aiera/client-sdk/modules/AieraChat/components/Messages/MessageFactory/TextBlock';
import {
    CHAT_SESSION_QUERY,
...
// Raw types from GraphQL (used internally, not exported)
type RawCitation = {
    __typename?: string;
    author?: string | null;
    date?: string | null;
 ...
 */
export function normalizeContentBlock(rawBlock: RawBlock): TextBlock | null {
    if (!rawBlock) {
        return null;
    }
    try {
        return {
            ...
            citations: rawBlock.citations ? rawBlock.citations.map((c) => normalizeCitation(c)) : undefined,
            id: generateId('block-text'), // Virtuoso requires that all messages have a unique ID
        };
    } catch (error) {
                ...
                if (chatSession.responseMessages) {
                    (chatSession.responseMessages as RawChatMessageResponse[]).forEach((msg) => {
                        ...
                        const blocks = Array.isArray(msg.blocks)
                            ? msg.blocks.map((block) => normalizeContentBlock(block as RawBlock)).filter(isNonNullable)
                            : [];
